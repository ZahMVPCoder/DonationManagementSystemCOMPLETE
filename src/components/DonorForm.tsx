...
export function DonorForm() {
  ...
  const existingDonor = isEditing ? mockDonors.find((d) => d.id === id) : null;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // POST /api/donors (create) or PATCH /api/donors/:id (update)
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
          ...
          <div className="mt-6 p-4 bg-blue-50 rounded-lg">
            <p className="text-sm text-blue-900">
              <strong>Note:</strong> This form currently uses mock data. Connect to your
              Prisma/Neon backend to persist changes.
            </p>
          </div>
        </form>
      </div>
